document.addEventListener("DOMContentLoaded", function () {
  ...
  function handleTimeOut() {
    ...
    if (selectedOption) {
      if (
        selectedOption.value === questions[currentQuestionIndex].correctAnswer
      ) {
        score++;
      }
  ...
  function displayScore() {
        ...
        if (question.question === selectedQuestion.question) {
          question.options.forEach((option) => {
            ...
            if (option === question.correctAnswer) {
              ...
              optionLabel.innerHTML = `<input type="radio" disabled ${
                option === selectedQuestion.selectedOption ? "checked" : ""
              }> <span>${option}</span>`;
    ...
    if (skippedQuestions.length > 0) {
      ...
      skippedQuestions.forEach((skippedQuestion, index) => {
        ...
        skippedQuestionText.textContent = `${index + 1}. ${
